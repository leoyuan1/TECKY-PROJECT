...
async function postPets(req: Request, res: Response) {
    try {
        ...
        if (speciesID == "define") {
            const alreadyExistSpecies = (
                await client.query("select id from species where species_name = $1", [adoption_species_name])
            ).rows[0];
            if (alreadyExistSpecies) {
                speciesID = alreadyExistSpecies.id;
            } else {
                speciesID = await client.query(
                    "insert into species (pet_type_id, species_name) values ($1, $2) returning id",
                    [adoption_pet_type, adoption_species_name]
                );
...
async function status(req: Request, res: Response) {
    ...
    if (existData.status == "active") {
        let resultStatus = (
            await client.query(`UPDATE posts SET status = $1, updated_at = now() WHERE id = $2 Returning status`, [
                "hidden",
                result,
            ])
        ).rows;
...
async function request(req: Request, res: Response) {
    ...
    let checkPostRequest = (
        await client.query(`select * from post_request where post_id = $1 and from_id = $2`, [postUser.id, existingUser.id])
    ).rows[0];
    if (checkPostRequest) {
        res.json({
            message: "requested",
...
async function changeRequestO(req: Request, res: Response) {
    ...
    let postStatus = await (
        await client.query(`update posts set status = $1 where id = $2 returning *`, ["adopted", nowStatus.post_id])
    ).rows[0];
    if (nowStatus.status == "waiting for approval") {
        let requestResult = (
            await client.query(`UPDATE post_request SET status = $1 where id = $2 returning *`, ["approval", result])
        ).rows[0];

        let otherRequest = (
            await client.query(`update post_request set status = $1 where post_id = $2 and id != $3 returning id,status`, [
                "not approval",
                nowStatus.post_id,
                result,
            ])
...
async function changeRequestX(req: Request, res: Response) {
    ...
    if (nowStatus.status == "waiting for approval") {
        let requestResult = (
            await client.query(`UPDATE post_request SET status = $1 where id = $2 returning *`, ["not approval", result])
        ).rows[0];
